...
```bash
ls "${DOCVAULT_PATH}/Projects/<name>/Security Reviews/"*.md 2>/dev/null | sort -r | head -1
```
...
```bash
# All open issues
grep -rl "status: backlog\|status: todo\|status: in-progress" ${DOCVAULT_PATH}/Projects/<name>/Issues/*.md 2>/dev/null
```
...
```
${DOCVAULT_PATH}/Projects/<name>/audit/<YYYY-MM-DD>-<HHMMSS>.md
```
...
```bash
cd ${DOCVAULT_PATH} && mkdir -p "Projects/<name>/audit" && git add "Projects/<name>/audit/" && git commit -m "audit: <project> <date>" && git push origin main
```
