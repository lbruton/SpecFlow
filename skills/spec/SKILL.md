...
- **MANDATORY — reconcile workflow vs code state:** In a git repo, `spec-status` includes a `divergence` block comparing the workflow state (gates, task markers, logs) against actual git state (commits on the branch). **If that block is present and `divergence.detected` is `true`, STOP.** A previous session committed code that ran ahead of the workflow (e.g. commits on the branch while the readiness gate is unapproved). Present the divergence reasons and the three options to the user — **(1)** catch up gates, **(2)** roll back, **(3)** continue as-is — and let them choose before doing any other work. Never silently continue from the code state. (If `spec-status` omits the block — e.g. the project isn't a git repo — there's nothing to reconcile; proceed normally.)
