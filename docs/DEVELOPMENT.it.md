...
1. **Registra in index** (`src/tools/index.ts`):

```typescript
export { myNewTool } from './my-new-tool';
```

1. **Aggiungi al server** (`src/index.ts`):
...
1. **Aggiungi JavaScript** (`dashboard/script.js`):
...
1. **Gestisci nel server** (`src/server.ts`):
