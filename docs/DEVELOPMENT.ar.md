...
1. **سجل في الفهرس** (`src/tools/index.ts`):

```typescript
export { myNewTool } from './my-new-tool';
```

1. **أضف إلى الخادم** (`src/index.ts`):
...
1. **أضف JavaScript** (`dashboard/script.js`):
...
1. **التعامل في الخادم** (`src/server.ts`):
