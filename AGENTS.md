...
- **CodeQL NOT required** - only `Codacy Static Code Analysis` is required by `protect-main` ruleset. Verify via `gh api repos/lbruton/specflow/rulesets`. Pre-existing CodeQL annotations on unchanged lines are pre-existing, not PR-introduced.
- **OAuth lacks `workflow` scope** - pushes touching `.github/workflows/` are rejected. `.github/` is in `.prettierignore` for this reason. File workflow changes as a separate issue.
