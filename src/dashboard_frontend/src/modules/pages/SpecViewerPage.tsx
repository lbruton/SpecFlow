    ...
    <div className="grid gap-4">
      <div className="panel p-4 flex items-center justify-between">
        <div className="font-semibold text-[var(--text-primary)]">
          ...
          <span className="text-[var(--text-secondary)]">{spec || t('common.unknown')}</span>
        </div>
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center bg-[var(--surface-sunken)] rounded-lg p-1">
            {(['discovery', 'requirements', 'design', 'tasks', 'readiness-report'] as const)
              .filter(
                (d) =>
                  (d !== 'readiness-report' || documents?.['readiness-report']) &&
                  (d !== 'discovery' || documents?.['discovery']),
              )
              .map((d) => (
                <button
                  key={d}
                ...
                >
                  {d === 'readiness-report'
                    ? t('specsPage.documents.readinessReport', 'Readiness')
