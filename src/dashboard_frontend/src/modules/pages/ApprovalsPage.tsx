...
function ApprovalItem({
  ...
  const { approvalsAction, getApprovalContent, getApprovalSnapshots, getApprovalDiff, reloadAll } =
    useApi();
  ...
  useEffect(() => {
    ...
    if (!a.filePath && a.content) {
      ...
      getApprovalContent(a.id)
        .then((res) => {
          if (active) setContent(String(res.content || ''));
        })
  ...
  const handleApprove = async () => {
    ...
    try {
      const result = await approvalsAction(a.id, 'approve', {
        response: t('approvalsPage.messages.approvedViaDashboard'),
      });
      if (result.ok) {
        ...
        showNotification(
          t('approvalsPage.notifications.approveFailed', { title: a.title }),
          'error',
        );
  ...
  const handleRejectWithFeedback = async (feedback: string) => {
    ...
    try {
      ...
      if (result.ok) {
        ...
        showNotification(
          t('approvalsPage.notifications.rejectFailed', { title: a.title }),
          'error',
        );
  ...
  const handleConcernsWithFeedback = async (feedback: string) => {
    ...
    try {
      ...
      if (result.ok) {
        ...
        showNotification(
          t('approvalsPage.notifications.concernsFailed', { title: a.title }),
          'error',
        );
  ...
  const handleRevision = async () => {
    ...
    try {
      ...
      if (result.ok) {
        showNotification(
          t('approvalsPage.notifications.revisionRequested', { title: a.title }),
          'success',
        );
        setOpen(false);
        setComments([]);
        ...
        showNotification(
          t('approvalsPage.notifications.revisionFailed', { title: a.title }),
          'error',
        );
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-3 text-sm mb-3">
              ...
              >
                ...
                {a.status === 'needs-revision'
                  ? t('approvals.status.needsRevision')
              ...
              <span className="text-[var(--text-faint)] flex items-center gap-1 font-mono tabular-nums">
                ...
                {formatDate(
                  a.createdAt,
                  { fallbackKey: 'common.unknown', fallbackText: 'Unknown' },
                  t,
                )}
              </span>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3 min-w-0">
              ...
              >
                ...
                <span className="hidden sm:inline">
                  {open
                    ? t('approvalsPage.actions.closeReview')
                    : t('approvalsPage.actions.openReview')}
                </span>
                <span className="sm:hidden">
                  {open ? t('common.close') : t('approvalsPage.actions.reviewShort')}
                </span>
              </button>
              ...
              <button
                ...
                title={
                  selectedCount > 1
                    ? t('approvalsPage.actions.useBatchActions')
                    : t('approvalsPage.actions.concernsTooltip')
                }
              ...
              {open && (
                ...
                >
                  ...
                  <span className="hidden sm:inline">
                    {t('approvalsPage.actions.requestRevisions')}
                  </span>
            ...
            <div className="space-y-2">
              <div className="flex items-center bg-[var(--surface-panel)] rounded-lg p-1 shadow-sm border border-[var(--border-default)]">
                ...
                >
                  ...
                  <span className="hidden sm:inline">
                    {t('approvals.annotator.sideBySide.buttonLabel')}
                  </span>
                </button>
                {snapshots.length > 0 && (
                  <button
                    ...
                    >
                      {snapshots.map((snapshot) => (
                        <option key={snapshot.version} value={snapshot.version}>
                          v{snapshot.version} - {snapshot.approvalTitle} -{' '}
                          {getSnapshotTriggerDescription(snapshot.trigger)} (
            ...
            <div>
              ...
              {!diffLoading && diff && hasDiffChanges(diff) && (
                <div>
                  ...
                  <DiffViewer diff={diff} viewMode={diffViewMode} showLineNumbers={true} />
                </div>
              )}
              ...
              {!diffLoading && diff && !hasDiffChanges(diff) && (
                <div className="text-center py-8 text-[var(--text-muted)]">
                  <svg
                    className="mx-auto w-8 h-8 text-[var(--text-faint)] mb-2"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
                  ...
                  <p className="text-sm">
                    The current version is identical to the selected snapshot.
                  </p>
                </div>
              )}
            </div>
...
function Content() {
  ...
  const executeBatchAction = useCallback(
    async (action: 'approve' | 'reject', feedback?: string) => {
      ...
      try {
        ...
        showNotification(
          'There was a problem processing the selected items. Please try again.',
          'error',
        );
      } finally {
  ...
  const handleBatchRejectWithFeedback = useCallback(
  ...
  const isAllSelected =
    filteredApprovals.length > 0 && selectedIds.size === filteredApprovals.length;
        ...
        <div className="flex flex-col gap-4">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              {filteredApprovals.length > 0 && (
                ...
                >
                  ...
                  <span className="hidden sm:inline">
                    {selectionMode
                      ? t('approvalsPage.selection.exitMode')
                      : t('approvalsPage.selection.enterMode')}
                  </span>
                  <span className="sm:hidden">
          ...
          {categories.length > 1 && (
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-end gap-3 overflow-x-hidden">
              <label
                htmlFor="approvals-filter"
                className="text-sm font-medium text-[var(--text-secondary)] whitespace-nowrap"
              >
                {t('approvalsPage.filter.label')}
              </label>
              <select
                id="approvals-filter"
              ...
              >
                {categories.map((cat) => (
                  <option key={cat} value={cat}>
                    {cat === 'all'
                      ? t('approvalsPage.filter.options.all')
                      : cat === 'steering'
                        ? t('approvalsPage.filter.options.steering')
                        : cat}
                  </option>
                ))}
              </select>
      ...
      {filteredApprovals.length === 0 ? (
        <div className="bg-[var(--surface-panel)] border border-[var(--border-default)] shadow rounded-lg p-3 sm:p-4 md:p-6 max-w-full overflow-x-hidden">
          <div className="text-center py-8 sm:py-12 text-[var(--text-muted)]">
            <svg
              className="mx-auto h-10 w-10 sm:h-12 sm:w-12 text-[var(--text-faint)] mb-3 sm:mb-4"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
            ...
            <p className="text-base sm:text-lg font-medium text-[var(--text-primary)] mb-1 sm:mb-2">
              {t('approvalsPage.empty.title')}
            </p>
            <p className="text-sm text-[var(--text-muted)] max-w-sm mx-auto">
              {t('approvalsPage.empty.description')}
            </p>
          </div>
        </div>
      ) : (
        ...
        message={t('approvalsPage.batchConfirm.message', {
          ...
          action:
            pendingAction === 'approve'
              ? t('approvalsPage.actions.approve').toLowerCase()
              : t('approvalsPage.actions.reject').toLowerCase(),
        })}
              ...
              <div className="flex-1">
                ...
                {batchResult.failed.length > 0 && (
                  <p className="text-xs text-yellow-700 dark:text-yellow-300 mt-1">
                    {t('approvalsPage.batchResult.failedCount', {
