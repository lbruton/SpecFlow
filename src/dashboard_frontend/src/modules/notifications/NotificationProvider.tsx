...
const NotificationActionsContext = createContext<NotificationActionsContextType | undefined>(
  undefined,
);
...
export function NotificationProvider({ children }: { children: React.ReactNode }) {
  console.log(
    '[NotificationProvider] ========== COMPONENT MOUNTED WITH DIAGNOSTIC CODE ==========',
  );
  ...
  const playNotificationSound = useCallback(() => {
    console.log(
      '[Audio] playNotificationSound called - soundEnabled:',
      soundEnabled,
      'volume:',
      volume,
    );
  ...
  const handleTaskUpdate = useCallback(
    async (specName: string, specDisplayName: string) => {
      try {
        ...
        if (prevTaskData && currentTaskData) {
          ...
          if (currentTaskData.inProgress !== prevTaskData.inProgress) {
            if (currentTaskData.inProgress && !prevTaskData.inProgress) {
              ...
              const task = currentTaskData.taskList?.find(
                (t: any) => t.id === taskId || t.number === taskId,
              );
          ...
          if (
            currentTaskData.completed === currentTaskData.total &&
