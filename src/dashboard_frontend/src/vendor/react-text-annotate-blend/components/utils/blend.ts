import { blend, range, sortBy } from './utils';
...
const updateIndices = (splits: Array<Tag>, blend: Array<Tag>) => {
  ...
  const tagRanges = splits
    .map((i, j) => {
      ...
      const semiInclusive: boolean = blendRange.includes(i.start) || blendRange.includes(i.end);

      const totalInclusive: boolean = tagRange.every((val: number) => blendRange.includes(val));

      const blendInclusive: boolean = blendRange.every((val: number) => tagRange.includes(val));
      if (semiInclusive || totalInclusive || blendInclusive) {
        tagIndices.push(j);
      }
      i.__index__ = j;

      if ((semiInclusive && !totalInclusive) || (blendInclusive && !totalInclusive)) {
        ...
        totalIncInds.push(
          splits.findIndex(
            (tag, index) => tag.start <= inclusive.start && tag.end >= inclusive.end,
          ),
        );
        return undefined;
      } else {
  ...
  const outRanges = tagRanges.map((val) => {
    ...
    blend.forEach((overlap, _) => {
      ...
      if (overlap.end - overlap.start === 1 && outRange.includes(overlap.start)) {
...
export const blender = (tags: Array<Tag>) => {
  ...
  } else return { tags: currentTags, blendIndices: [] as number[] };
};
