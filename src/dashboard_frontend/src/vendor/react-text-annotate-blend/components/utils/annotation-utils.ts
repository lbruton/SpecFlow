...
export const generalSplitClick = <T extends Span>(
  ...
  if (splitIndex >= 0) {
    tagTransformer(
      [...currentTags.slice(0, splitIndex), ...currentTags.slice(splitIndex + 1)],
      onChange,
      overLapLimit,
    );
  }
  ...
  if (isBlendable) {
    ...
    if (frontOverlapIndex >= 0) {
      tagTransformer(
        [...currentTags.slice(0, frontOverlapIndex), ...currentTags.slice(frontOverlapIndex + 1)],
        onChange,
        overLapLimit,
      );
    } else if (rearOverlapIndex >= 0) {
      tagTransformer(
        [...currentTags.slice(0, rearOverlapIndex), ...currentTags.slice(rearOverlapIndex + 1)],
        onChange,
        overLapLimit,
      );
    }
...
export const generalHandleMouseUp = <T extends Span>(
  ...
  if (selection && selection.anchorNode && selection?.focusNode) {
    ...
    const startBase = selection.anchorNode.parentElement?.getAttribute('data-start');
    const endBase = selection.focusNode.parentElement?.getAttribute('data-start');
