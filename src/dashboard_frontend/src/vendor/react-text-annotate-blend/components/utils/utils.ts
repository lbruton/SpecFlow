...
// Local replacement for lodash.sortby (SFLW-47 vendoring): supports the two
// call forms used in this module tree — an iteratee fn or a single-key array.
// Array.prototype.sort is stable in ES2019+, matching lodash semantics here.
export function sortBy<T>(
  collection: T[],
  iteratee: ((item: T) => number | string) | Array<keyof T>,
): T[] {
  const getKey =
    typeof iteratee === 'function'
      ? iteratee
      : (item: T) => item[iteratee[0]] as unknown as number | string;
...
export const strictSplitWithOffsets = (text: string, offsets: { start: number; end: number }[]) => {
  let lastEnd = 0;
  const splits = [];
  for (let offset of sortBy(offsets, (o) => o.start)) {
    const { start, end } = offset;
