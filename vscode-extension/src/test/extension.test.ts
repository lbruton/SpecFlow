...
suite('Extension Test Suite', () => {
  ...
  suite('Approval Notification Logic Tests', () => {
    ...
    test('should NOT detect status changes as new approvals', () => {
      ...
      assert.strictEqual(
        newIds.length,
        0,
        'Should NOT detect any new approvals when only status changes',
      );
    });
    ...
    test('should NOT detect false positives when previous list is properly initialized', () => {
      ...
      assert.strictEqual(
        newIds.length,
        0,
        'Should NOT detect any new approvals when lists are identical',
      );
    });
    ...
    test('should detect ALL as new when previous list is empty (edge case)', () => {
      ...
      assert.strictEqual(
        newIds.length,
        2,
        'Empty previous list causes all to appear new (bug scenario)',
      );
    });
  ...
  suite('Batch Operation Undo Logic Tests', () => {
    ...
    function canPerformUndo(
      ...
      return lastOperation !== null && lastOperation.ids.length > 0 && undoWindowActive;
    }
